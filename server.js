...
const isAuthenticated = (req) => {
    ...
    return Boolean(cookies.cms_session && sessions.has(cookies.cms_session));
};
...
const writeProjects = async (projects) => {
    await fsp.writeFile(path.join(root, "proyectos.json"), `${JSON.stringify(projects, null, 2)}\n`);
};
...
const writeServices = async (services) => {
    await fsp.writeFile(path.join(root, "servicios.json"), `${JSON.stringify(services, null, 2)}\n`);
};
...
const buildProjectPayload = async (fields, files, currentProject = null) => {
    ...
    if (image && image.content.length) {
        ...
        await fsp.mkdir(path.join(root, "assets", "proyectos"), { recursive: true });
...
const listProjects = async (req, res) => {
    if (!isAuthenticated(req)) {
        return sendJson(res, 401, { message: "Inicia sesion para administrar proyectos." });
    }
...
const saveProject = async (req, res) => {
    if (!isAuthenticated(req)) {
        return sendJson(res, 401, { message: "Inicia sesion para guardar proyectos." });
    }
...
const updateProject = async (req, res, id) => {
    if (!isAuthenticated(req)) {
        return sendJson(res, 401, { message: "Inicia sesion para editar proyectos." });
    }
    ...
    return sendJson(res, 200, { message: "Proyecto actualizado.", project: projects[index] });
};

const deleteProject = async (req, res, id) => {
    if (!isAuthenticated(req)) {
        return sendJson(res, 401, { message: "Inicia sesion para eliminar proyectos." });
    }
...
const sanitizeList = (value, maxItems = 8, maxLength = 180) => {
    ...
    return source.map((item) => sanitizeText(item, maxLength)).filter(Boolean).slice(0, maxItems);
};
...
const buildServicePayload = (body, currentService = null) => {
    ...
    if (!title || !category || !summary || !description || !highlights.length || !deliverables.length || !tags.length) {
        const error = new Error("Completa titulo, categoria, descripciones, listas y etiquetas.");
        error.statusCode = 400;
        throw error;
    }
...
const listServices = async (req, res) => {
    if (!isAuthenticated(req)) {
        return sendJson(res, 401, { message: "Inicia sesion para administrar servicios." });
    }
...
const saveService = async (req, res) => {
    if (!isAuthenticated(req)) {
        return sendJson(res, 401, { message: "Inicia sesion para guardar servicios." });
    }
...
const updateService = async (req, res, id) => {
    if (!isAuthenticated(req)) {
        return sendJson(res, 401, { message: "Inicia sesion para editar servicios." });
    }
    ...
    return sendJson(res, 200, { message: "Servicio actualizado.", service: services[index] });
};

const deleteService = async (req, res, id) => {
    if (!isAuthenticated(req)) {
        return sendJson(res, 401, { message: "Inicia sesion para eliminar servicios." });
    }
...
const handleLogin = async (req, res) => {
    ...
    sessions.add(token);
    res.setHeader("Set-Cookie", `cms_session=${token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=28800`);
...
const server = http.createServer(async (req, res) => {
    try {
        const requestUrl = new URL(req.url, `http://${host}:${port}`);
        const projectRoute = /^\/api\/proyectos\/([^/]+)$/.exec(requestUrl.pathname);
        const serviceRoute = /^\/api\/servicios\/([^/]+)$/.exec(requestUrl.pathname);
        ...
        console.error(error);
        return sendJson(res, error.statusCode || 500, { message: error.message || "No pudimos procesar la solicitud." });
    }
});
...
server.listen(port, host, () => {
    ...
    console.log("Password CMS por defecto: avora-admin");
});
